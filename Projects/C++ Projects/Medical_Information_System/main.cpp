...
public:
    ...
    string getTestName(){ return testName;}
    virtual ~baseTestRequest() = default;
};
...
public:
    ...
    virtual ~baseClinic(){
        ...
        for (auto* request : requiredTests) delete request;
    }
...
public:
    ...
    void execute(){
        ...
        for(std::size_t i=0; i<testsOfPatient->size(); ++i)
        {
...
public:
    ...
    void acceptRequest(secretaryCommand* newCommand) {
        if(newCommand->getCommandName()=="seeDoctor") {
            ...
            for(std::size_t i=0; i<previousCommandsByCurrentPatient.size(); ++i) {
...
public:
    ...
    void addPatientToRecord(patient* patientToAdd) { //'Attach' from observer pattern
        ...
        for(std::size_t i=0; i<drugsOfPatient->size(); ++i) {
    ...
    void informAllPatients(int whichDrug) { //inform all drug owners about side effects 'Update' function
        for(std::size_t i=0; i<drugOwners[whichDrug].size(); ++i){
