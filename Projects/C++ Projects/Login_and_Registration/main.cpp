...
{
    if (!firstname.empty() && std::isupper(static_cast<unsigned char>(firstname.front())))
      return true;
	  else
	  return false;
}
...
{
    if (!lastname.empty() && std::isupper(static_cast<unsigned char>(lastname.front())))
      return true;
...
bool validEmail(string email) // function to check the validity of the email
{
    ...
    std::string::size_type AT = std::string::npos, dot = std::string::npos;                                                            // variables to store the position of ( @ )and (dot)
    ...
    {
        for (std::size_t i = 0; i < email.length(); ++i)
        {
        ...
        if (AT == std::string::npos || dot == std::string::npos) // If (@) or (dot)is not present
